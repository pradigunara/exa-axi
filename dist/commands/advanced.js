...
function validateDate(val, flagName) {
    if (!val)
        return undefined;
    if (!DATE_RE.test(val)) {
        throw new AxiError(`Invalid date format for ${flagName}: ${val}. Must be YYYY-MM-DD`, "VALIDATION_ERROR", [`Example: --${flagName} 2025-01-15`]);
    }
    return val;
}
